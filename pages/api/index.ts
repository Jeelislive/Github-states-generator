...
import { generateStatsCard } from "@/lib/svg";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { username, theme = "default", show_icons, hide_border, hide_progress, show, bg_color, title_color } = req.query;
  ...
  try {
    ...
    const showArray = show ? (typeof show === "string" ? show.split(",") : []) : [];
    const svg = generateStatsCard(stats, selectedTheme, {
      showIcons: show_icons === "true",
      hideBorder: hide_border === "true",
      hideProgress: hide_progress === "true",
      show: showArray,
    });
